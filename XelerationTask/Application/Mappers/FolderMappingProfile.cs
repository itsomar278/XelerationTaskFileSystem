...
namespace XelerationTask.Application.Mappers
{
    public class FolderMappingProfile : Profile
    {
        public FolderMappingProfile() {
           ...
           .ForMember(dest => dest.ParentFolder, opt => opt.MapFrom(src =>
               src.ParentFolder == null ? null : new IdNameDto { Id = src.ParentFolder.Id, Name = src.ParentFolder.Name }))

           .ForMember(dest => dest.ChildFolders, opt => opt.MapFrom(src =>
               src.SubFolders.Select(f => new IdNameDto { Id = f.Id, Name = f.Name }).ToList()))

           .ForMember(dest => dest.ChildFiles, opt => opt.MapFrom(src =>
